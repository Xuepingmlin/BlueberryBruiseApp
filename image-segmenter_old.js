...
//const imageSize = 512
const imageSize_0 =300 //for berry detection
...
window.runModel = async function () {
  ...
  Rect.sort((a, b) => (a.y > b.y) ? 1 : -1 )
   varmeanHeight = Rect.reduce(function(prev, cur) {return prev + cur.height; }, 0)/Rect.length;
  Rect_1=[];
  k_temp=[];
  RRect=[]
  k_temp.push(0)
...
function ResizeImageData(imageData,w,h){
  ...
  scalecanvas,width=w/canvas.width;
